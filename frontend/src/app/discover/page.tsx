...
import { Compass, Star, LayoutGrid } from 'lucide-react';
...
export default function DiscoverPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const res = await fetch(`http://localhost:8000/api/discover?category=${activeTab}`);
                ...
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm transition-colors ${activeTab === 'topics' ? 'bg-zinc-800 text-cyan-400 border border-cyan-900/30' : 'hover:bg-zinc-800 text-zinc-400'}`}
                        >
                            <LayoutGrid size={16} />
                            <span>Topics</span>
                        </button>
